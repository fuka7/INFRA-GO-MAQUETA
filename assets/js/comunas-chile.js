...
var COMUNAS_CHILE = {
  ...
  "Región de O'Higgins": [
    'Chimbarongo','Chépica','Codegua','Coínco','Coltauco','Doñihue',
    ...
    'Requínoa','San Fernando','San Francisco de Mostazal','San Vicente',
    'Santa Cruz'
  ],
  'Región del Maule': [
  ...
  'Región del Biobío': [
    'Antuco','Arauco','Biobío','Cañete','Concepción','Contulmo','Coronel',
    'Curanilahue','Florida','Hualpén','Hualqui','Laja','Lebu',
    'Los Álamos','Los Ángeles','Lota','Mulchén','Nacimiento','Negrete',
    'Penco','Quilaco','Quilleco','San Pedro de la Paz','San Rosendo',
    'Santa Bárbara','Talcahuano','Tirúa','Tomé','Tucapel','Yumbel'
  ],
  ...
  'Región de Los Lagos': [
    'Ancud','Calbuco','Castro','Chaitén','Cochamó','Curaco de Vélez',
...
window.igcLoadComunas = function(regionVal, comunaActual) {
  var select = document.getElementById('igcCoComuna');
  if (!select) return;

  var comunas = COMUNAS_CHILE[regionVal] || [];
  ...
  comunas.forEach(function(c) {
    ...
    opt.value = c;
    opt.textContent = c;
    if (comunaActual && c === comunaActual) opt.selected = true;
